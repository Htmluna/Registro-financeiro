...
def decimal_field_validator(form, field):
    ...
    try:
        ...
        # usando a função auxiliar 'valor_para_decimal' (definida no final deste arquivo ou importada).
...
class ContaForm(FlaskForm):
    ...
    nome = StringField('Nome da Conta', validators=[DataRequired(message="O nome da conta é obrigatório."), Length(max=100)])
    # Campo para o valor. Usa StringField para aceitar formatos com vírgula/ponto.
    # Obrigatório. Usa o validador customizado 'decimal_field_validator'.
    valor = StringField('Valor', validators=[DataRequired(message="O valor é obrigatório."), decimal_field_validator])
    # Campo para a data de vencimento. Obrigatório. Espera formato YYYY-MM-DD.
    vencimento = DateField('Vencimento', format='%Y-%m-%d', validators=[DataRequired(message="A data de vencimento é obrigatória.")])
    ...
    parcela_atual = IntegerField('Parcela Atual', validators=[Optional(), NumberRange(min=1, message="Parcela deve ser 1 ou maior.")])
    # Campo para o total de parcelas. Opcional, mas se preenchido, deve ser >= 1.
    total_parcelas = IntegerField('Total de Parcelas', validators=[Optional(), NumberRange(min=1, message="Total de parcelas deve ser 1 ou maior.")])
    ...
    tipo_pagamento = SelectField('Pagar com / Associar a', coerce=int, validators=[Optional()]) # Texto do label mais explicativo
...
class CartaoForm(FlaskForm):
    ...
    nome = StringField('Nome do Cartão', validators=[DataRequired(message="O nome do cartão é obrigatório."), Length(max=50)])
    # Campo para o limite total. Usa StringField para a máscara e o validador decimal customizado. Obrigatório.
    limite = StringField('Limite Total', validators=[DataRequired(message="O limite é obrigatório."), decimal_field_validator])
    # Botão de submissão.
...
class ContaBancariaForm(FlaskForm):
    ...
    nome = StringField('Nome da Conta', validators=[DataRequired(message="O nome da conta é obrigatório."), Length(max=50)])
    # Campo para o saldo atual. Usa StringField para a máscara e o validador decimal customizado. Obrigatório.
    saldo = StringField('Saldo Atual', validators=[DataRequired(message="O saldo é obrigatório."), decimal_field_validator])
    # Botão de submissão.
    submit = SubmitField('Salvar Conta')
...
# --- Definição da Função `valor_para_decimal` ---
# Incluída aqui para garantir que o validador `decimal_field_validator` a encontre.
# Idealmente, esta função estaria em um arquivo `utils.py` e seria importada aqui e em `app.py`.
import decimal # Garante que decimal está importado neste escopo também
